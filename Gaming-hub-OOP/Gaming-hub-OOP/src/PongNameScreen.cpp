...
PongNameScreen::PongNameScreen()
{
    ...
    promptText.setPosition(280, 230);
...
void PongNameScreen::reset()
{
    ...
    stage = 1;                         
    promptText.setString("Enter Player 1 Name:");
}

void PongNameScreen::handleEvent(RenderWindow& window, Event& event, GameState& screen)
{
        ...
        else if (event.text.unicode == '\r' || event.text.unicode == '\n')  
        {
            if (!currentInput.empty())
            {
                if (stage == 1)
                {
                    ...
                    inputDisplay.setString("");
                    stage = 2;
                    promptText.setString("Enter Player 2 Name:");
                }
                else if (stage == 2)
                {
                    ...
                    screen = PONG_SCREEN;
                    cout << "DEBUG: switched to PONG_SCREEN"<<endl;
                }
            }
        }
...
void PongNameScreen::draw(RenderWindow& window)
{
